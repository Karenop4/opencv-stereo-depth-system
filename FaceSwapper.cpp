...
void FaceSwapper::swapFaces(cv::Mat& frame, const cv::Rect& rect_ann, const cv::Rect& rect_bob)
{
...
cv::Mat FaceSwapper::getMinFrame(const cv::Mat& frame, const cv::Rect& rect_ann, const cv::Rect& rect_bob)
{
...
void FaceSwapper::getFacePoints(const cv::Mat& frame)
{
    ...
    feather_amount.width = std::max(1, feather_amount.width);
    feather_amount.height = std::max(1, feather_amount.height);
}
...
{
    specifyHistogram(small_frame(big_rect_ann), warpped_faces(big_rect_ann), warpped_mask_bob(big_rect_ann));
    specifyHistogram(small_frame(big_rect_bob), warpped_faces(big_rect_bob), warpped_mask_ann(big_rect_bob));
}

void FaceSwapper::featherMask(cv::Mat& refined_masks)
{
    cv::erode(refined_masks, refined_masks, cv::getStructuringElement(cv::MORPH_RECT, feather_amount), cv::Point(-1, -1), 1, cv::BORDER_CONSTANT, cv::Scalar(0));

    cv::blur(refined_masks, refined_masks, feather_amount, cv::Point(-1, -1), cv::BORDER_CONSTANT);
}

void FaceSwapper::pasteFacesOnFrame()
{
    for (int i = 0; i < small_frame.rows; i++)
    {
        ...
        for (int j = 0; j < small_frame.cols; j++)
        {
            if (*masks_pixel != 0)
            {
                const int alpha = *masks_pixel;
                frame_pixel[0] = static_cast<uchar>(((255 - alpha) * frame_pixel[0] + alpha * faces_pixel[0]) >> 8);
                frame_pixel[1] = static_cast<uchar>(((255 - alpha) * frame_pixel[1] + alpha * faces_pixel[1]) >> 8);
                frame_pixel[2] = static_cast<uchar>(((255 - alpha) * frame_pixel[2] + alpha * faces_pixel[2]) >> 8);
            }
...
void FaceSwapper::specifyHistogram(const cv::Mat& source_image, cv::Mat target_image, const cv::Mat& mask)
{
